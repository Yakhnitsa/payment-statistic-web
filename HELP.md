...
- Запускаем приложение с девелоперскими пропертями.
 ...
 - Выделяем запросы Hibernate по созданию базы данных:
     ...
     Hibernate: create table payment_details (id bigint not null, additional_payment bigint not null, dateStamp dateStamp, document_number varchar(255), income_type integer, payment bigint not null, payment_code varchar(255), payment_description varchar(255), station_code integer not null, station_name varchar(255), tax_payment bigint not null, total_payment bigint not null, type varchar(255), number integer, payer_code integer, primary key (id))
